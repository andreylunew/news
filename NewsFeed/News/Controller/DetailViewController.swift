...
class DetailViewController: UIViewController {
	...
	private func updateNewsData() {
        NetworkManager.shared.getNewsContent(urlSlug: news.descript!) { (result) in //urlSlug
			switch result {
	...
	private func loadFromCache() {
		guard let cachedNews = CoreDataStack.shared.fetch(NewsEntity.self, title: self.news.title!) else { return }
				DispatchQueue.main.async { [weak self] in
	...
	private func update(content: String, for news: NewsEntity) {
		guard let result = CoreDataStack.shared.fetch(NewsEntity.self, title: news.title!) else {
			return
		}
